...
The map is a two-layer canvas. The basemap and the hot pixels go into an
offscreen buffer that is only redrawn when the view, the data or the theme
changes; the visible canvas is one `drawImage` plus the handful of animated
rings. Mid-gesture, even that buffer is not re-rendered — plate carrée is an
affine projection, so the previous buffer is translated and scaled into place
and the real redraw happens once the gesture settles.
...
screen-space thinning keeps only the strongest detection per 3-pixel cell, and
marks are batched into one path per shape-and-power-bucket rather than a fill
and a stroke each.

The basemap gets the same treatment: per-arc, per-landmass and per-lake
bounding boxes computed at decode let the renderer skip everything off screen,
features under about 1.5 px are dropped, and a stride walks the vertex list
when the simplification tolerance is well under a pixel. Subpath count, not
vertex count, is what an even-odd fill actually charges for — 900 landmasses
cost 21 ms and 400 cost 7 ms with the same stride — which is why the threshold
is tuned in pixels rather than degrees.
...
detections as at 5 000, and panning is a constant-cost blit. The canvas backing
store is also hard-capped at ~4.2 megapixels, stepping the pixel ratio down
rather than handing a phone a canvas it cannot composite in a frame.
